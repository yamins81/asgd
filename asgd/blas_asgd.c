	...
	bool feedback)
{
	...
	data->sgd_weights = matrix_init(1, n_feats, 0.0f);
	...
	data->asgd_weights = matrix_init(1, n_feats, 0.0f);
	...
	for (size_t i = 0; i < X->rows; ++i) {
		...
		if (margin < 1)
		{
			...
			cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
					...
					1.0f,
					data->sgd_weights->data, data->sgd_weights->cols);
	...
	matrix_t *r)
{
	...
	cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
		r->rows, r->cols, data->asgd_weights->cols, 1.0f,
